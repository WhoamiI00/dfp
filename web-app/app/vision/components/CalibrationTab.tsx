...
import MovementPlansPanel from "./MovementPlansPanel";
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-2 rounded border border-amber-500/30 bg-amber-500/5 p-4">
        ...
        <p className="text-sm text-white/70">
          Inject intrinsics and extrinsics for a top-down overhead camera using
          the current frame&apos;s dimensions. Use after uploading a test image
          (CameraSourceBar at the top of the page) to skip real calibration.
        </p>
        <button
          type="button"
          disabled={busy}
      ...
      <div className="border border-white/10 rounded">
        <button
          type="button"
          onClick={() => setShowRealCamera(s => !s)}
          className="w-full text-left p-3 text-sm text-white/70 hover:text-white"
        >
          {showRealCamera ? "▼" : "▶"} Real-camera setup (chessboard + ArUco corners)
        </button>
        {showRealCamera && (
